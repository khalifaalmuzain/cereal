...
def get_cereal_manufacturer_search(request):
	...
	cereals = Cereal.objects.filter(name__istartswith=cereal)
	manufacturer = Manufacturer.objects.filter(name__istartswith=manufacturer)

	cereal_manufacturer = """
	<form action='/get_cereal_manufacturer_search/' method='GET'>
	...
	if manufacturer != 'None' and cereal != 'None':
		...
		for manufacturer in manufacturer:
			...
			for cereal in cereals:
				cereal_manufacturer += "%s ,</br> %s,</br> %s</br>" % (cereal.name, manufacturer.name, cereal.nutrition.nutrition_value())
...
def manu_list_view(request):
	...
	for manufacturer in manufacturers:
		...
		manu_string +="<h4>%s</h4>" % manufacturer
